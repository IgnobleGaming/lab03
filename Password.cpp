...
void Password::guess(int try_password, int num_matches)
{
	...
	ListArrayIterator<String>* itor = viable_words->iterator();
	ListArray<String>* new_viable_words = new ListArray<String>();
	while(itor->hasNext())
	{
		String* viableWord = itor->next();
		if(num_matches==getNumMatches(viableWord, guess)) new_viable_words->add(viableWord);
	}
...
int Password::getNumMatches(String* curr_word, String* word_guess)
{
	...
	for(int i = 0; i < len; i++)
	{
		if(curr_word->charAt(i) == word_guess->charAt(i)) counter++;
	}
...
void Password::displayViableWords()
{
	...
	while(iter->hasNext())
	{
		std::cout << iter->next()->getText() << std::endl;
	}
...
{
	//Returns word at index if exists array bounds
	if(index > 0 && index < all_words->size())return all_words->get(index);
}